...
<body>
	...
	<div class="chart"style="background-color:white;width: 100%; height: 100vh;">
		<div class="charts-card" id="data"> 
			<p class="chart-title" style="text-align: center;"><h1>STATISTIQUE SUR LES STOCK SORT </h1></p>
	...
	/*chart*/
		  ...
		  var barChartOptions = {
 			...
 			<%
	 	       ...
	 	       String readData = "SELECT nom_prodt,SUM(quantiter)AS sm FROM sortant GROUP by nom_prodt";
	 	       Statement st = con.createStatement();
	 	
	 	       rss = st.executeQuery(readData);
	 	       
	 	       while (rss.next()) {
	 	           String id = rss.getString("nom_prodt");
	   		%>
		  			<%=rss.getString("sm")  %>,
  			<% } %>
		  		]
		  	...
		  	xaxis: {
		  	  ...
		  	  categories: [
		  		<%
		 	       ...
		 	       String Dt = "SELECT nom_prodt FROM sortant GROUP by nom_prodt";
		 	       Statement ts = con.createStatement();
		 	
		 	       ra = st.executeQuery(readData);
		 	       
		 	       while (ra.next()) {
		 	           String id = ra.getString("nom_prodt");
		   		%>
		  			"<%=ra.getString("nom_prodt") %>",
	  			<% } %>
		  	  ],
		  	},
		  	yaxis: {
